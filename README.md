...
3. **PostInstall** — Extracts the `vp` binary, creates `vpx`/`vpr` multicall symlinks, writes a `package.json` wrapper plus an `.npmrc` pinned to the public npm registry, runs `vp install` to bootstrap JS dependencies (isolated from the caller’s PATH and `~/.npmrc`), and sets up the `~/.vite-plus/current` symlink.
